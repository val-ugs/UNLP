...
def convert_nlp_dataset_to_df(nlp_dataset):
    ...
    for nlp_text in nlp_texts:
        ...
        data.append([nlp_text.text, nlp_text.classification_label, [nlp_token.token for nlp_token in nlp_tokens], ner_tags])
    df = pd.DataFrame(data, columns=['text', 'labels', 'tokens', 'tags'])
    return df