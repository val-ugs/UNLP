...
from apps.common.models import NlpDataset, NlpText, NlpToken, NlpTokenNerLabel
...
def calculate_ner_metric(request):
    try:
        ...
        nlp_dataset = get_object_or_404(NlpDataset, pk=nlp_dataset_pk)
        df = convert_nlp_dataset_to_df(nlp_dataset)
        list_of_tags = [tag for tag_list in df["tags"].tolist() for tag in tag_list]
        id2label, label2id = get_id2label_label2id(list_of_tags)
