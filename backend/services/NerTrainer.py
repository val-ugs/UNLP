...
class NerTrainer:
    def __init__(self, model_name_or_path, training_args, metric, train_df, val_df):
        ...
        tokenized_train_dataset = self.preparation.get_ner_dataset(train_df)
        tokenized_val_dataset = self.preparation.get_ner_dataset(val_df)
    ...
    def train(self, path_to_model=None):
        self.trainer.train()
        if path_to_model:
            self.trainer.save_model(path_to_model)

    def predict(self, test_df):
        test_dataset = self.preparation.get_ner_dataset(test_df)
