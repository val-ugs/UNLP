...
def define_nlp_token_ner_label(nlp_token, row, ner_labels, is_finished_ner_labels, total_added_tokens):
    if (is_finished_ner_labels[NerLabel.DateTime] == False):
        ...
        if (total_added_tokens >= len(row['Time']) + len(row['Level']) + len(row['Content']) - 1):
...
def process_apache(token_pattern_to_remove, token_pattern_to_split, ner_labels):
    ...
    for _, row in df.iterrows():
        ...
        nlp_texts.append({
            ...
            'text': lines[lineId],
            'classification_label': ClassificationLabel.Apache.value,
            'summarization': None
        })
