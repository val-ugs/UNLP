...
class WindowsLogNlpDatasetParser(LogNlpDatasetParser):
    def init_log_and_csv_files(self):
        log_file_name = os.path.join(os.path.dirname(__file__), '../../logs/Windows_2k.log')
        csv_file_name = os.path.join(os.path.dirname(__file__), '../../logs/Windows_2k.log_structured.csv')
        return log_file_name, csv_file_name
    
    def init_classification_label(self):
        return ClassificationLabel.Windows.value
